...
def comprimir(texto, tabla_frecuencias):
    ...
    for caracter in texto:
        datos_comprimidos.extend(tabla_codigos[caracter])
    return datos_comprimidos

def decodificar_arbol_huffman(bit_array, arbol_huffman):
    texto = ''
...
def guardar_archivo(data, extension):
    ...
    if ruta_archivo:
        with open(ruta_archivo, 'wb') as archivo:  # Abre el archivo en modo binario
            archivo.write(data.tobytes())  # Escribe los bytes en el archivo
...
def descompactar_archivo():
    ...
    if ruta_archivo:
        with open(ruta_archivo, 'rb') as archivo:
            datos_comprimidos = bitarray()
            datos_comprimidos.frombytes(archivo.read())
